...
function Item:GetInfo()
	local temp = self:GetFrame():GetItemInfo(self:GetBag(), self:GetID())
	local localItemId = C_Container.GetContainerItemID(self:GetBag(), self:GetID())
	if localItemId ~= nil then
		local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemIcon = GetItemInfo(localItemId)
		temp.quality = itemQuality
		temp.id = localItemId
		temp.link = itemLink
	end
	return temp
